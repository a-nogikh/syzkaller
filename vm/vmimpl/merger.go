...
type OutputMerger struct {
	Output   chan Chunk
	decoders sync.Map // map[string]chan error
	teeMu    sync.Mutex
	tee      io.Writer
	wg       sync.WaitGroup
}
...
// Errors() returns a channel that will receive errors from the curretly active decoders.
func (merger *OutputMerger) Errors(ctx context.Context) <-chan error {
	eg, egCtx := errgroup.WithContext(ctx)
	merger.decoders.Range(func(_, value any) bool {
		ch := value.(chan error)
		eg.Go(func() error {
			select {
			case <-egCtx.Done():
...
func (merger *OutputMerger) AddDecoder(name string, typ OutputType, r io.ReadCloser,
	...
	merger.decoders.Store(name, done)
	merger.wg.Add(1)
	go func() {
		defer merger.wg.Done()
		defer close(done)
		err := merger.runDecoder(typ, r, decoder)
		done <- MergerError{name, r, err}
	}()
}
