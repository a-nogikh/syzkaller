...
func TestTreeOriginLtsBisection(t *testing.T) {
	...
	// TODO: once the info is shared, replace the check with public API calls.
	bug := ctx.loadBug()
	assert.Len(t, bug.FixCandidates, 1)
	candidate := bug.FixCandidates[0]
	candidate.JobKey = ""
	assert.Equal(t, candidate, BugFixCandidate{
		Source: BisectionFixCandidate,
		Repo:   "https://upstream.repo/repo",
