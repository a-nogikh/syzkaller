...
func TestOpenBugSubsystemRefresh(t *testing.T) {
	c := NewCtx(t)
	defer c.Close()
	...
	subsystem := &entity.Subsystem{
		Name:      "first",
		PathRules: []entity.PathRule{{IncludeRegexp: `test\.c`}},
	}
	c.setSubsystems(ns, []*entity.Subsystem{subsystem})

	// Refresh subsystems.
	_, err := c.AuthGET(AccessUser, "/refresh_subsystems")
	c.expectOK(err)
	...
	// Update subsystems once more.
	subsystem.Name = "second"
	c.setSubsystems(ns, []*entity.Subsystem{subsystem})
	...
	_, err = c.AuthGET(AccessUser, "/refresh_subsystems")
	c.expectOK(err)
	expectSubsystems(t, client, extID, "second")
}
...
func TestClosedBugSubsystemRefresh(t *testing.T) {
	...
	// Wait until the refresh period is over.
	c.advanceTime(fixedBugsUpdateTime - openBugsUpdateTime)
