...
func TestDiscussionAccess(t *testing.T) {
	...
	// First bug's reporting stage with discussion is hidden.
	// So we should display no discussions at all when AccessPublic.
	got := getBugDiscussionsUI(c.ctx, firstBug, AccessPublic)
	if diff := cmp.Diff([]*uiBugDiscussion(nil), got); diff != "" {
		t.Fatal(diff)
	}

	// But for AccessADmin we need to show the patch.
	got = getBugDiscussionsUI(c.ctx, firstBug, AccessAdmin)
	...
	// For second bug, both discussions are bound to the public reporting stage.
	got = getBugDiscussionsUI(c.ctx, secondBug, AccessPublic)
	if diff := cmp.Diff([]*uiBugDiscussion{
		{
			Subject:  "Second bug reported",
...
Hello`, msg.Sender)
	...
	got := getBugDiscussionsUI(c.ctx, bug, AccessPublic)
	if diff := cmp.Diff([]*uiBugDiscussion{
		{
			Subject:  "Bug reported",
...
Hello`, msg.Sender)
	_, err = c.POST("/_ah/mail/", incoming2)
	c.expectOK(err)
...
func TestEmailUnrelatedDiscussions(t *testing.T) {
	c := NewCtx(t)
	defer c.Close()
	...
	// An email that's not Cc'd to the target mailing list.
...
In-Reply-To: <1234>
From: user@user.com
To: %v, linux-kernel@vger.kernel.org
Content-Type: text/plain
