...
func apiCandidateSeedsPoll(ctx context.Context, ns string, req *dashapi.CandidateSeedsPollReq) (any, error) {
	seeds, maxCreatedAt, lastID, err := aidb.PollCandidateSeeds(ctx, ns, req.Name, req.TargetOS, req.TargetArch, 20)
	if err != nil {
		return nil, err
	}
	resp := &dashapi.CandidateSeedsPollResp{
		Token: encodeCandidateSeedsToken(maxCreatedAt, lastID),
	}
	for _, s := range seeds {
