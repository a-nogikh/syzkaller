...
func crossTreeBisection(c context.Context, bug *Bug,
	...
	err = repoGraph.forEachEdge(func(from, to *repoNode, info KernelRepoLink) error {
		...
		newJob := &Job{
			...
			KernelCommit:    crashBuild.KernelCommit,
			...
			// Don't report such job results separately.
			// The results will get conveyed via bug.FixCandidates.
			Reported: true,
		}
...
func (bug *Bug) addCandidate(candidate *BugFixCandidate) {
	for _, item := range bug.FixCandidates {
		if item.Source == candidate.Source &&
			item.Repo == candidate.Repo &&
			item.Branch == candidate.Branch &&
			item.Commit.Title == candidate.Commit.Title {
			return
		}
	}
	bug.FixCandidates = append(bug.FixCandidates, *candidate)
}
...
func doneCrossTreeBisection(c context.Context, jobKey *db.Key, job *Job) error {
	...
	return updateSingleBug(c, jobKey.Parent(), func(bug *Bug) error {
		bug.addCandidate(&BugFixCandidate{
			Source: BisectionFixCandidate,
			Repo:   job.KernelRepo,
			Branch: job.KernelBranch,
			Commit: job.Commits[0],
		})
		return nil
	})
}
