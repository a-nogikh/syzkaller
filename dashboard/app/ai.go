...
func buildPatchHistory(ctx context.Context, job *aidb.Job) ([]ai.PatchHistoryEntry, error) {
	...
	var parentDiff, parentDesc string
	currentJobID := reporting.JobID
	for currentJobID != "" {
		j, err := aidb.LoadJob(ctx, currentJobID)
		if err != nil || j == nil {
			break
		}
		if j.Results.Valid {
			if m, ok := j.Results.Value.(map[string]any); ok {
				if d, _ := m["PatchDiff"].(string); d != "" {
					parentDiff = d
					parentDesc, _ = m["PatchDescription"].(string)
					break // Found the closest patch!
				}
			}
		}
