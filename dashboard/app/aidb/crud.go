...
type AlreadyUpstreamedError struct {
	Stage string
}

func (e *AlreadyUpstreamedError) Error() string {
	return fmt.Sprintf("already upstreamed at stage %s", e.Stage)
}
...
func checkNoParallelConflict(ctx context.Context, txn *spanner.ReadWriteTransaction, job *Job, stage string) error {
	...
	if len(conflicts) > 0 {
		return &AlreadyUpstreamedError{Stage: stage}
	}
	return nil
}
...
func AddJobReportingTransactional(ctx context.Context, job *Job, entry *JobReporting, noParallel bool) error {
	...
	if err != nil {
		if spanner.ErrCode(err) == codes.AlreadyExists {
			return &AlreadyUpstreamedError{Stage: entry.Stage}
		}
		return err
	}
	return nil
