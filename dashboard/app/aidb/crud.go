...
func CreatePatchIterationJob(ctx context.Context, reportingID string, debounce time.Duration) (*Job, error) {
	...
	_, err = client.ReadWriteTransaction(ctx, func(ctx context.Context, txn *spanner.ReadWriteTransaction) error {
		...
		running, err := checkSiblingJobs(ctx, txn, reporting.ID)
		if err != nil {
			return err
		}
		if running {
...
func checkSiblingJobs(ctx context.Context, txn *spanner.ReadWriteTransaction, parentReportingID string) (bool, error) {
	stmt := spanner.Statement{
	...
	stmt := spanner.Statement{
		SQL: `SELECT JobReporting.ID FROM JobReporting 
              ...
              WHERE Jobs.BugID = @bugID 
                ...
                AND COALESCE(JSON_VALUE(Jobs.Results, '$.PatchDiff'), '') != ''
              LIMIT 1`,
		Params: map[string]any{
			"bugID":     bugID,
			"createdAt": createdAt,
...
func getUnprocessedComments(ctx context.Context, txn *spanner.ReadWriteTransaction,
	...
	commentIter := txn.Query(ctx, stmt)
	defer commentIter.Stop()
	var commentIDs []string
	for {
		cRow, err := commentIter.Next()
		...
		var cid string
		if err := cRow.Column(0, &cid); err != nil {
			return nil, err
		}
...
func MarkCommentsProcessed(ctx context.Context, ids []string) error {
	client, err := dbClient(ctx)
	if err != nil {
		return err
	}
	_, err = client.ReadWriteTransaction(ctx, func(ctx context.Context,
		tx *spanner.ReadWriteTransaction) error {
		var mutations []*spanner.Mutation
		for _, id := range ids {
			mutations = append(mutations, spanner.Update("JobComments", []string{"ID", "Processed"}, []any{id, true}))
		}
