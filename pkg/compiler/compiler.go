...
func (comp *compiler) parseAttrs(descs map[string]*attrDesc, parent ast.Node, attrs []*ast.Type) (
	map[*attrDesc]uint64, map[*attrDesc]prog.ValueExpression) {
	_, parentType, parentName := parent.Info()
	resInt := make(map[*attrDesc]uint64)
	resExpr := make(map[*attrDesc]prog.ValueExpression)
...
func (comp *compiler) parseAttrExprArg(attr *ast.Type) prog.ValueExpression {
	if len(attr.Args) != 1 {
	...
	arg := attr.Args[0]
	if arg.HasString {
		// TODO: also support if[field] or if[int].
		comp.error(attr.Pos, "%v argument must be an expression", attr.Ident)
		return nil
	}
	return comp.genValueExpression(arg)
}
