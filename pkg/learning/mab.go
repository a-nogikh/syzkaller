...
func (b *baseMAB[T]) Action(r *rand.Rand, exploration float64) Action[T] {
	...
	if r.Float64() < exploration {
		...
		randPrefix := r.Float64() * b.totalWeight
		pos = b.prefixes.findPrefix(randPrefix)
	}
	if pos >= len(b.arms) {
		// There must always be such an element since we passed
		// the prefix less than the total sum.
		panic("inconsistent MAB")
	}
	return Action[T]{
		Arm:   b.arms[pos],
		index: pos,
...
type fenwickTree[T int | float64] struct {
	elements []T
}
