...
var resolveFixes = aflow.NewFuncAction("resolve-fixes", func(ctx *aflow.Context, args struct {
	...
	FixesHash   string
	UpdateFixes bool
}) (struct{ Fixes ai.FixesTag }, error) {
	if !args.UpdateFixes {
		return struct{ Fixes ai.FixesTag }{args.BaseFixes}, nil
	}
	if args.FixesHash == "" {
		return struct{ Fixes ai.FixesTag }{}, nil
	}
	fix, err := queryFixesTag(ctx, args.FixesHash)
	return struct{ Fixes ai.FixesTag }{fix}, err
})
