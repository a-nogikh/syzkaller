...
func (s *DiffFuzzerStore) SaveRepro(result *ReproResult) {
	now := time.Now().Unix()
	origTitle := result.Crash.Report.Title

	crashLog := fmt.Sprintf("%v.crash.log", now)
	s.saveFile(origTitle, crashLog, result.Crash.Output)
	log.Logf(0, "%q: saved crash log into %s", origTitle, crashLog)

	s.patch(origTitle, func(obj *DiffBug) {
		if result.Repro != nil {
			obj.Patched.Repro = s.saveFile(origTitle,
				reproFileName,
				result.Repro.Prog.Serialize())
		}
		if result.Stats != nil {
			reproLog := fmt.Sprintf("%v.repro.log", now)
			obj.Patched.ReproLog = s.saveFile(origTitle, reproLog, result.Stats.FullLog())
			log.Logf(0, "%q: saved repro log into %s", origTitle, reproLog)
		}
	})
}
