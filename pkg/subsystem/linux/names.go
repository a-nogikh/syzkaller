...
func setSubsystemNames(list []*setNameRequest) error {
	// TODO: use the # of matching files to determine those subsystems
	// that deserve to take a name in case of a name collision.
	...
	for _, req := range list {
		...
		if req.referenceEmail != "" {
			...
			if name != "" && !dups[name] && validateName(name) {
