...
// This code is based on the what has already been in place for a long time.
// We may make better fuzzing decisions if we also remember per-program
// coverage. Or we may not - we need to try and see.
...
func (corpus *Corpus) AddRawMaxSignal(signal []uint32, prio uint8) bool {
	corpus.mu.Lock()
	defer corpus.mu.Unlock()
...
func (corpus *Corpus) signalDiff(sign signal.Signal) signal.Signal {
	corpus.mu.RLock()
	defer corpus.mu.RUnlock()
	return corpus.signal.Diff(sign)
}
