...
type Fuzzer struct {
	...
	fuzzReturns learning.WindowRanker[float64]
}
...
func NewFuzzer(ctx context.Context, cfg *Config, rnd *rand.Rand,
	...
	f := &Fuzzer{
		...
		fuzzReturns: learning.WindowRanker[float64]{Size: 25000},
	}
...
func (fuzzer *Fuzzer) handleMABs(req *Request, res *Result, elapsedSec float64, newSignal int) {
	...
	reward := currSpeed
	if req.softMax != nil {
		fuzzer.Config.Corpus.RandomSoftMaxDone(*req.softMax,
			fuzzer.fuzzReturns.RatioLessThan(currSpeed))
	}
	...
	if req.stat == statFuzz {
		fuzzer.fuzzReturns.Save(currSpeed)
...
func (fuzzer *Fuzzer) LogMAB() {
	...
	for {
		...
		i++
		if i%3 == 0 {
			log.Logf(0, "distribution: %+v", fuzzer.fuzzReturns.Distribution())
		}
	}
}
