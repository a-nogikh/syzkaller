...
func (fuzzer *Fuzzer) execute(executor queue.Executor, req *queue.Request, opts ...execOpt) *queue.Result {
	...
	res := queue.Execute(fuzzer.ctx, executor, req)
...
func (fuzzer *Fuzzer) logCurrentStats() {
	for {
		...
		for _, item := range []logQueue{
			{fuzzer.smashQueue, "smash"},
			{fuzzer.triageQueue, "triage"},
			{fuzzer.candidateQueue, "candidate"},
			{fuzzer.triageCandidateQueue, "triage candidate"}} {
			str = fmt.Sprintf("%s, %s: %d queued", str, item.name, item.q.Len())
		}
		fuzzer.Logf(0, "%s", str)
	}
}
