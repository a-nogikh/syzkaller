...
func TestFuzz(t *testing.T) {
	...
	tf.registerExecutor(proc.execute)
	tf.wait()
...
func TestFuzzerRaces(t *testing.T) {
	...
	// Based on the example in Go source tests.
	crc32q := crc32.MakeTable(0xD5828281)

	tf := newTestFuzzer(t, fuzzer, nil, 25000)
	for i := 0; i < 8; i++ {
		tf.registerExecutor(func(req *Request) (*Result, string, error) {
			...
			for i, call := range req.Prog.Calls {
				cover := uint32(call.Meta.ID*1024) +
					uint32(crc32.Checksum(serializedLines[i], crc32q)%4)
...
func newTestFuzzer(t *testing.T, fuzzer *Fuzzer, expectedCrashes map[string]bool, iterLimit int) *testFuzzer {
	return &testFuzzer{
		t:               t,
		fuzzer:          fuzzer,
...
func (f *testFuzzer) registerExecutor(cb func(*Request) (*Result, string, error)) {
	f.eg.Go(func() error {
		for f.oneMore() {
			req := f.fuzzer.NextInput()
			res, crash, err := cb(req)
			if err != nil {
				return err
			}
			if crash != "" {
