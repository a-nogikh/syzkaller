...
func parseSyzMountImage(c *prog.Call) (*mountImageArgs, error) {
	...
	if segmentsCountArg == nil || !ok {
		return nil, fmt.Errorf("segment count was expected to be const")
	}
	sizeArg, ok := c.Args[2].(*prog.ConstArg)
	if sizeArg == nil || !ok {
		return nil, fmt.Errorf("invalid size arg")
	}
	segmentsPtrArg, ok := c.Args[4].(*prog.PointerArg)
	if segmentsPtrArg == nil || segmentsPtrArg.Res == nil || !ok {
