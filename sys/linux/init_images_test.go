...
func TestExtractSyzMountImage(t *testing.T) {
	...
	files, err := filepath.Glob("testdata/fs_images/*.in")
	if err != nil {
	...
	for _, file := range files {
		...
		for idx, mount := range p.MountedImages() {
			outFilePath := fmt.Sprintf("%s.out%d", base, idx)
			var testResult []byte
			if mount.Reader != nil {
				var err error
				testResult, err = io.ReadAll(mount.Reader)
				if err != nil {
					t.Fatal(err)
				}
			}
			if *flagUpdate && mount.Reader != nil {
			...
			if outExists && mount.Reader == nil {
				t.Fatalf("#%d: mount not found, but exists in the answer", idx)
			} else if !outExists && mount.Reader != nil {
				t.Fatalf("#%d: mount found, but does not exist in the answer", idx)
			}
			if testResult != nil {
