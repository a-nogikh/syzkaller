...
func main() {
	...
	stream := queue.NewLimitedQueue(4096)
	base.source = stream
...
func (dc *diffContext) Loop(ctx context.Context) {
	...
	go func() {
		// Let both base and patched instances somewhat progress in fuzzing.
	...
	freqStat := time.NewTicker(10 * time.Second)
	...
	for {
		...
		case <-freqStat.C:
			log.Logf(0, "base [%s], patched [%s]; reproducing %d",
				dc.base.stats(), dc.new.stats(), len(reproLoop.Reproducing()),
			)
		...
		case rep := <-dc.new.crashes:
			...
			log.Logf(0, "patched crashed: %v [need repro = %v]",
				rep.Title, need)
			dc.store.PatchedCrashed(rep.Title, rep.Report)
			if need {
...
func (rr *runRepro) Run(r *repro.Result) {
	...
	ret := runReproResult{originalTitle: r.Report.Title}
	...
	for i := 0; i < 3; i++ {
		...
		if i <= 1 {
			...
			// Just in case the bug reproduces without Threaded.
			opts.Repeat = true
		}
		...
		log.Logf(1, "attempt #%d to run %q on base: crashed=%v", i, r.Report.Title, crashed)
		if crashed {
...
func (kc *kernelContext) MachineChecked(features flatrpc.Feature, syscalls map[*prog.Syscall]bool) queue.Source {
	...
	if kc.source == nil {
		source = queue.Duplicator(kc.setupFuzzer(features, syscalls), kc.duplicateInto)
	} else {
		source = kc.source
	}
...
func (kc *kernelContext) setupFuzzer(features flatrpc.Feature, syscalls map[*prog.Syscall]bool) queue.Source {
	...
	var fuzzPCs map[uint64]bool
	if !kc.cfg.FuzzFilter.Empty() {
		filter, _, err := manager.CreateCoverageFilter(kc.reportGenerator, kc.cfg.FuzzFilter)
		if err != nil {
			log.Fatalf("failed to init fuzz coverage filter: %v", err)
		}
		fuzzPCs = map[uint64]bool{}
		for _, pc := range filter {
			fuzzPCs[pc] = true
		}
		log.Logf(0, "%s: fuzz PCs size: %d", kc.name, len(fuzzPCs))
	}
...
func (kc *kernelContext) CoverageFilter(modules []*vminfo.KernelModule) []uint64 {
	...
	execFilter, _, err := manager.CreateCoverageFilter(kc.reportGenerator, kc.cfg.CovFilter)
	if err != nil {
		log.Fatalf("failed to init coverage filter: %v", err)
	}
	log.Logf(0, "cover filter size: %d", len(execFilter))
	if kc.http != nil {
...
func (kc *kernelContext) stats() string {
	stat := fmt.Sprintf("execs: %d, VMs: %d",
		kc.servStats.StatExecs.Val(),
		kc.servStats.StatNumFuzzing.Val(),
	)
	if fuzzer := kc.fuzzer.Load(); fuzzer != nil {
		stat += fmt.Sprintf(", corpus: %d/%d, signal: %d",
			kc.corpus.StatFuzzProgs.Val(), kc.corpus.StatProgs.Val(),
