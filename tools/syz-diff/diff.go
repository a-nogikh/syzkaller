...
func setup(ctx context.Context, name string, cfg *mgrconfig.Config) *kernelContext {
	...
	var tags []string
	for _, item := range cfg.Experimental.FocusOrder {
	...
	kernelCtx := &kernelContext{
		...
		ctx:             ctx,
		cfg:             cfg,
		corpus:          corpus.NewTaggedCorpus(ctx, tags, nil),
...
func (kc *kernelContext) setupFuzzer(features flatrpc.Feature, syscalls map[*prog.Syscall]bool) queue.Source {
	var focusOrder []*fuzzer.FocusConfig
	for _, item := range kc.cfg.Experimental.FocusOrder {
		_, filter, err := manager.CreateCoverageFilter(kc.reportGenerator, item.Filter, false)
		if err != nil {
			log.Fatalf("failed to init fuzz coverage filter %s: %v", item.Name, err)
		}
		coverPCs := manager.PCsToCover(kc.cfg, filter)
		log.Logf(0, "%s: focus PCs size: %d", item.Name, len(coverPCs))
		focusOrder = append(focusOrder, &fuzzer.FocusConfig{Name: item.Name, CoverPCs: coverPCs})
	}
...
func (kc *kernelContext) CoverageFilter(modules []*vminfo.KernelModule) []uint64 {
	...
	execFilter, _, err := manager.CreateCoverageFilter(kc.reportGenerator, kc.cfg.CovFilter, true)
	if err != nil {
