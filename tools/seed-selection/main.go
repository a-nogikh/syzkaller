...
var targetDist map[uint64]int64

func main() {
	fuzzDistPath := flag.String("fuzz_distribution", "", "path to the baseline distribution file (0xaddr;count)")
	flag.Parse()
	args := flag.Args()
	if len(args) < 1 {
		fmt.Println("usage: go run main.go [--fuzz_distribution file.txt] <filename>")
		return
	}
	result, err := parseHexFile(args[0])
	if err != nil {
	...
	if *fuzzDistPath != "" {
		var err error
		targetDist, err = parseDistributionFile(*fuzzDistPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "error loading distribution: %v\n", err)
			os.Exit(1)
		}
		fmt.Printf("Loaded baseline distribution with %d PCs\n", len(targetDist))
	}
...
func measureDistribution(dist map[uint64]int64) {
	...
	if targetDist != nil {
		sim := cosineSimilarity(dist, targetDist)
		fmt.Printf("Cosine Similarity to Baseline: %.4f\n", sim)
		sim = filteredCosineSimilarity(dist, targetDist)
		fmt.Printf("Filtered Cosine Similarity to Baseline: %.4f\n", sim)
	}
...
func filteredCosineSimilarity(fuzzerDist, baselineDist map[uint64]int64) float64 {
	var dotProduct, magFuzzer, magBaseline float64

	for addr, countFuzzer := range fuzzerDist {
		valFuzzer := float64(countFuzzer)
		magFuzzer += valFuzzer * valFuzzer

		// Only look at the baseline if it exists in our current fuzzer results
		if countBaseline, ok := baselineDist[addr]; ok {
			valBaseline := float64(countBaseline)
			dotProduct += valFuzzer * valBaseline
			magBaseline += valBaseline * valBaseline
		}
	}

	if magFuzzer == 0 || magBaseline == 0 {
		return 0
	}

	return dotProduct / (math.Sqrt(magFuzzer) * math.Sqrt(magBaseline))
}

func cosineSimilarity(a, b map[uint64]int64) float64 {
	var dotProduct, magA, magB float64
	for addr, countA := range a {
		valA := float64(countA)
		if countB, ok := b[addr]; ok {
			dotProduct += valA * float64(countB)
		}
		magA += valA * valA
	}
...
func parseDistributionFile(filename string) (map[uint64]int64, error) {
	file, err := os.Open(filename)
	if err != nil {
		return nil, err
	}
