...
func main() {
	...
	fuzzerObj := fuzzer.NewFuzzer(context.Background(), &fuzzer.Config{
		...
		NeedCandidates: make(chan struct{}),
...
func (tool *FuzzerTool) pollLoop() {
	...
	for {
		...
		select {
		case <-ticker:
		case <-tool.fuzzer.Config.NeedCandidates:
			needCandidates = true
		}
		...
		if needCandidates || time.Since(lastPoll) > 10*time.Second*tool.timeouts.Scale {
			needCandidates = tool.fuzzer.NeedCandidates()
...
func (tool *FuzzerTool) poll(needCandidates bool, stats map[string]uint64) bool {
	...
	for _, inp := range r.Candidates {
		tool.addCandidate(inp)
	}
...
func (tool *FuzzerTool) addCandidate(candidate rpctype.Candidate) {
	p := tool.deserializeInput(candidate.Prog)
	if p == nil {
		return
	}
	tool.fuzzer.AddCandidate(fuzzer.Candidate{
		Prog:      p,
