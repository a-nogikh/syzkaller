...
type FuzzerTool struct {
	...
	inputs    chan rpctype.ExecutionRequest
	results   chan rpctype.ExecutionResult
...
func main() {
	...
	fuzzerTool := &FuzzerTool{
		...
		inputs:  make(chan rpctype.ExecutionRequest, inputsCount),
		results: make(chan rpctype.ExecutionResult, inputsCount),
	}
	...
	// Set up communication workers.
	for i := 0; i < 2; i++ {
		go fuzzerTool.exchangeDataWorker()
	}

	// The tool is not supposed to finish normally. Just hang.
	select {}
}
...
func (tool *FuzzerTool) exchangeDataWorker() {
	for result := range tool.results {
		results := []rpctype.ExecutionResult{
			result,
		}
