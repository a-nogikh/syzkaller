...
type FuzzerTool struct {
	...
	candidates *queueToChan[fuzzer.Candidate]
...
func main() {
	...
	fuzzerTool := &FuzzerTool{
		...
		candidates: newQueueToChan[fuzzer.Candidate](
			fuzzerObj.Config.Candidates, *flagProcs),
	}
	go fuzzerTool.candidates.run(context.Background())
...
func (tool *FuzzerTool) pollLoop() {
	...
	for {
		...
		if needCandidates ||
			time.Since(lastPoll) > 10*time.Second*tool.timeouts.Scale {
			select {
			case <-tool.candidates.needMore:
				needCandidates = true
			default:
...
func (tool *FuzzerTool) poll(needCandidates bool, stats map[string]uint64) bool {
	...
	tool.saveCandidates(r.Candidates)
...
func (tool *FuzzerTool) sendInputsWorker() {
	for inp := range tool.fuzzer.NewInputs {
...
func (tool *FuzzerTool) saveCandidates(candidates []rpctype.Candidate) {
	var newList []fuzzer.Candidate
	for _, candidate := range candidates {
		p := tool.deserializeInput(candidate.Prog)
		if p == nil {
			continue
		}
		newList = append(newList, fuzzer.Candidate{
			Prog:      p,
			Smashed:   candidate.Smashed,
			Minimized: candidate.Minimized,
		})
	}
	tool.candidates.addList(newList)
}
...
// queueToChan issues notification when the in channel capacity falls
// below the target.
type queueToChan[T any] struct {
	threshold   int
	buffer      []T
	cond        sync.Cond
...
func newQueueToChan[T any](out chan<- T, threshold int) *queueToChan[T] {
	return &queueToChan[T]{
		threshold: threshold,
		cond:      sync.Cond{L: &sync.Mutex{}},
		needMore:  make(chan struct{}, 1),
		out:       out,
	}
}

func (q *queueToChan[T]) addList(elems []T) {
	q.cond.L.Lock()
	defer q.cond.L.Unlock()
	q.buffer = append(q.buffer, elems...)
	q.needMoreSet = false
	q.cond.Broadcast()
}

func (q *queueToChan[T]) run(c context.Context) {
	go func() {
		<-c.Done()
		q.cond.Broadcast()
	}()
	for c.Err() == nil {
		q.cond.L.Lock()
		if len(q.buffer) < q.threshold && !q.needMoreSet {
			q.needMoreSet = true
			select {
			case q.needMore <- struct{}{}:
			default:
			}
		}
		for len(q.buffer) == 0 {
		...
		q.buffer = q.buffer[0 : len(q.buffer)-1]
		q.cond.L.Unlock()
		q.out <- elem
	}
}
