...
func OldStyleCollide(origProg *Prog, rand *rand.Rand) (*Prog, error) {
	if len(origProg.Calls)*2 > MaxCalls {
		return nil, fmt.Errorf("the prog is too big for the OldStyleCollide transformation")
	}
	prog := origProg.Clone()
	...
	for _, c := range dupCalls {
		...
		// Make it async with a 50% probability.
		if rand.Intn(2) != 0 {
			c.Props.Async = true
			leftAsync--
		}
