...
func (mgr *Manager) initRPCServer(ctx context.Context) {
	...
	if mgr.cfg.Snapshot {
		snapCfg := snapshotserver.Config{
			ExecutorBin:      mgr.cfg.ExecutorBin,
			UseCoverEdges:    mgr.cfg.Experimental.CoverEdges,
			Kernel64Bit:      mgr.cfg.SysTarget.PtrSize == 8,
			Slowdown:         mgr.cfg.Timeouts.Slowdown,
			SyscallTimeoutMs: int(mgr.cfg.Timeouts.Syscall / time.Millisecond),
			ProgramTimeoutMs: int(mgr.cfg.Timeouts.Program / time.Millisecond),
			Features:         mgr.enabledFeatures,
			SandboxArg:       mgr.cfg.SandboxArg,
			Stats:            mgr.servStats,
		}
		mgr.serv = snapshotserver.New(mgr.serv, snapCfg)
	}
...
func (mgr *Manager) fuzzerInstance(ctx context.Context, inst *vm.Instance, updInfo dispatcher.UpdateInfo) {
	...
	reps, err := serv.RunRequests(ctx, inst, mgr.reporter, injectExec, updInfo)
