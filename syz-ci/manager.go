...
func (mgr *Manager) uploadBuild(info *BuildInfo, imageDir string) (string, error) {
	...
	if mgr.storage != nil && assetUploadNeeded {
		...
		// In almost all cases it's ok if we first report the build and then upload
		// the assets - the probability that a crash happens and gets reported in between
		// are very low.
		go func() {
			...
			mgr.storage.ReportBuildAssets(build, assets...)
		}()
	}
	return build.ID, nil
}
