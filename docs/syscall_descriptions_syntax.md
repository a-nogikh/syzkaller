...
You may specify conditions that determine whether a field will be included, see
...
You may specify conditions that determine what field must be selected. If they are not
specified, syzkaller will randomly try all fields.
...
packet {
  ...
  integer int64  (if[value[header:haveField] == 0x1])
  body    array[int8]
} [packed]
...
if `header.haveInteger == 1`. That corresponds to e.g. the following program:
```
some_call(&AUTO={{AUTO, 0x1}, @value=0xabcd, []})
```

If `header.haveInteger` is not `1`, syzkaller will just pretend that the field
`integer` does not exist -- much like it's done for `optional[T]`.
```
some_call(&AUTO={{AUTO, 0x0}, @void, []})
```

A structure that has conditional fields is assumed to be `[varlen]` and.  If
a field of a `varlen` type or a conditional field appears in the middle of a
structure, the structure must be `[packed].`
...
In this case, the specific union field will be selected depending on the value
of the `type` field. For example, if `type` is `0x1`, then it's `int`:
```
...
The following definition would fail to compile:
...
sub_struct {
  f0 int
  f1 int (if[value[struct:f2]]) # Reference a field in a parent struct.
}
