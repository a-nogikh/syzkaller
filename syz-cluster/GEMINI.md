...
* `controller` API tests are in `pkg/controller/api_test.go`. The test logic in `pkg/service` components should be kept minimal and focus purely on isolated unit tests; do not write component tests with Spanner emulators or HTTP requests within `pkg/service`, instead write them in `pkg/controller/api_test.go` and use the defined API.
